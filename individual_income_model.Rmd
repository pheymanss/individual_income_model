...
With that, we went from 517 to 218 variables, a much more manageable amount of columns to consider in our model. We can now proceed to decode all categorical variables: the strategy here is to fetch the values specified in the data dictionary, and rewrite the values of each column by reference for the minimum memory overhead possible.
...
Most variables with high NA coverage are ones for which there exists some sort of "No data/Does not apply" value, but it is not explicitly stated, instead it i left empty  and subsequently read ans an NA by R (which is a reasonable space saving decision.) This means that there is no need to discard most of these variables, since they are easily mapped into the default response 'b' == 'Does not apply 'for categorical variables, and 0 would be appropriate for most numerical variables since those represent costs, payments and counts.
...
Recomputing the valid data coverage metrics, we see much better metric for most variables, only keeping the categorical variables that are not mappable to a 'b' field, and our target variable that we explicitly decided not to impute.
...
We will however remove all components of the PINCP variable present in the training data, since those variables are almost give out an explicit sum to get our target variable, and hence defeat the purpose of the exercise.

```{r discard_wage_earnings_variables}
dat <- discard_cols(dt = dat, cols = c('PERNP', 'WAGP', 'OIP', 'PAP', 'RETP', 'SEMP', 'SSIP',  'SSP', 'WAGP', 'WKHP'))
```


### Consistency within houshold income

It is also necessary to verify that the household information is consistent with the income of each of its members. The fist check should be to verify that all members of the same housing unit report the same household income.
...
This returns an empty query, which means that all housing units have only one housing. Next, we should verify that 
the individual income makes sense in the context of their housing unit. One might be inclined to verify that every individual's income is smaller than income of the the housing unit it resides, however as we saw in the exploration of our target variable, individual income can be positive, and hence one or more individual's income can be larger than their household's income.

The correct way to review this consistency is to verify that the sum of the individual incomes is equal to the housing income, including negative incomes 
...
## Numerical variables
 ...
 We can quickly skim through all numerical variables with the skim() function. Categorical variables are also supported by the function, but are omitted to avoid a very large table output.
...
dat[, hu_per_capita_income := HINCP/NP
  ...
  ][, perc_commuting_work := JWMNP/WKWN]
```
...
And we can see that the performance on the unseen data is virtually identical to the cross validation metrics. The model is consistent across unseen data, but the error metrics are not impressive.
...
```
The model is seeing modestly good results in the upper income bands, but does quite poorly in the lower ones. It is visible that the model had smaller errors on the lower bands, but not enough to overcome the higher penalty that the smaller bands entail in regards to relative error.
...
pred_frame[, good_prediction := relative_error < .2]

pred_frame[, .(perc_good_pred = mean(good_prediction)), by = income_band][order(income_band)] %>% knitr::kable()
```

In conclusion, our model is modestly competent and stable across the higher positive income bands, while taking a hit on the lower income bands.
...
The model relied heavily in the household and family income, and their respective per-capita approximations. These were expected to be our most robust proxies for the personal income, and then hours worked and age which stands to reason that they will positively influence individual income. 
...
*  Perform more complex household metrics like the count of different profiles by age, occupation, education level, and so on. For example, count of college educated adults by household.
